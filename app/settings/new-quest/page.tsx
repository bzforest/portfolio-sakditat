...
export default function AddQuestPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result.success) {
      alert('Quest successfully added to the Realm!');
      router.push('/'); // Or redirect to a list of projects
    } else {
      setError(result.error || 'Failed to add quest.');
    }
    setLoading(false);
  };

  if (!isAuthorized) return null; // Prevent flicker before redirect
      ...
      <main suppressHydrationWarning className="min-h-screen pt-24 pb-20 px-4 flex justify-center">
        <div className="w-full max-w-3xl bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl p-8 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl dark:shadow-[0_0_40px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-3xl font-bold text-slate-800 dark:text-white">📝 Forge New Quest</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input required type="text" name="title" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white focus:ring-1 focus:ring-amber-500 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">URL Slug * (e.g., vibe-cart)</label>
                <input required type="text" name="slug" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white focus:ring-1 focus:ring-amber-500 outline-none" />
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <select name="questType" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white outline-none">
              ...
              <div>
                ...
                <label className="flex items-center space-x-3 cursor-pointer mt-3">
                  <input type="checkbox" name="isPinned" className="w-5 h-5 accent-amber-500" />
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" name="role" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white focus:ring-1 focus:ring-amber-500 outline-none" />
              </div>
              <div>
                ...
                <input type="text" name="timeline" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white focus:ring-1 focus:ring-amber-500 outline-none" />
              </div>
            </div>
            ...
            <div>
              ...
              <input type="text" name="skills" placeholder="Next.js, Tailwind CSS, TypeScript" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white focus:ring-1 focus:ring-amber-500 outline-none" />
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Quest Lore (Description)</label>
              <textarea name="lore" rows={4} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white focus:ring-1 focus:ring-amber-500 outline-none resize-none"></textarea>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div>
                ...
                <input type="url" name="demoLink" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Frontend GitHub URL</label>
                <input type="url" name="frontendLink" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Backend GitHub URL</label>
                <input type="url" name="backendLink" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-800 dark:text-white outline-none" />
              </div>
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Project Image</label>
            ...
            <button disabled={loading} type="submit" className="w-full py-4 bg-amber-600 text-white font-bold rounded-xl hover:bg-amber-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
              {loading ? 'Forging Quest...' : 'Add Project to Realm'}
            </button>
